...
- Open the bare production URL (no `?harness=1`, no `?transport=`). The header badge must read
...
| 2 | *Categorise E-020 as travel.* | A blue card: "Set category of E-020 (Parking near the venue, 15.00 EUR) to travel". Click **Approve**. The table row updates, the card collapses into a receipt, the agent reports success. | "A write tool does not execute. It proposes. The page holds the call until I decide." | 25 |
